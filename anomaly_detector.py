...
def evaluar_anomalia(frecuencias, amplitudes, frecuencias_teoricas, tolerancia):
    """
    Evalúa los Top 5 picos de mayor amplitud comparándolos con las frecuencias teóricas puras.
    """
    indices_picos = encontrar_picos_numpy(amplitudes)
    
    # Filtro de corte en 65 Hz. Ignora completamente el 1X (29.5 Hz)
    ...
    # Comprobar la tolerancia (2%) contra las frecuencias teóricas para los 5 peaks
    for idx in top_indices:
        ...
        for nombre_falla, frec_teorica in frecuencias_teoricas.items():
            ...
            # Si alguno de los top 5 coincide con la frecuencia teórica, confirmar anomalía
            if limite_inferior <= frec_candidata <= limite_superior:
                return True, nombre_falla, frec_candidata, amp_candidata
                
    # Si no hubo match, retorna el peak #1 absoluto para registro visual sin anomalía
