...
4. **Detección Automática de Anomalías:** Identificación de las frecuencias dominantes mediante un buscador de picos nativo. El algoritmo aplica un filtro dinámico inferior para evadir el ruido de giro (1X) y su primer armónico (2X) (ej. > 65.0 Hz), evaluando el Top 12 de picos de mayor energía contra la cinemática teórica del rodamiento (BPFI, BPFO, BSF) con un margen de tolerancia estricto.
...
* `config.py`: Definición de hiperparámetros (frecuencia de muestreo, niveles `J`, multiplicadores cinemáticos, RPM y umbrales de tolerancia).
...
* `anomaly_detector.py`: Algoritmo detector de máximos locales e identificador de anomalías mediante ranking de energía (Top 12).
* `visualization.py`: Construcción del dashboard de 4 subgráficos para contrastar la señal temporal y el espectro del envolvente. Genera y guarda las visualizaciones.
...
3. Descarga los archivos `.mat` correspondientes al dataset CWRU y colócalos en la raíz del proyecto junto a los scripts.
4. Ejecuta el orquestador principal desde la terminal:
   `python main.py`
5. El sistema procesará secuencialmente cada archivo `.mat` encontrado, mostrará los logs de entropía y picos detectados por consola, y guardará automáticamente un dashboard gráfico en formato `.png` (`resultados_*.png`) en el mismo directorio.

## Resultados Visuales

El script genera un dashboard (guardado como `.png`) que consta de:
1. Serie Temporal Cruda Original.
2. Espectro de Envolvente Crudo (demostrando la ausencia de picos claros sin filtrar).
3. Serie Temporal Óptima Filtrada (Sub-banda de menor entropía), igualada a la escala de amplitud original para apreciar visualmente la reducción del ruido.
4. Espectro de Envolvente Óptimo, incluyendo el sombreado del área de límite de ruido dinámico ignorada y un marcador resaltando la frecuencia y amplitud exactas del pico dominante detectado (alineado con la frecuencia teórica de falla).